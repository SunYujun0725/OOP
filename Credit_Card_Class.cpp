...
int sumofDoubleEvenPlace(const string& buffer){  //偶數項*2相加
    ...
    if(length%2==0){  //14 or 16個數
        for(int i=0;i<length;i+=2){
            EvenSum+=getDigit(((int)buffer[i]-48)*2); //將字元轉成字串然後*2 去判斷是否大於10
        }
    }
    else{    //13 or 15個數
...
int cardtype(const string& buffer){
    if(buffer[0]=='4'){
        cout << "Visa card #"<< endl;
    }
    else if(buffer[0]=='5'){
...
string random(long MIN,long MAX){ 
    ...
    if(isvalid(strrandnumber)==1){   //隨機產生的數本身就符合
        ...
        //cout <<"ss"<< strrandnumber <<endl;
        long Complement=10-((sumofDoubleEvenPlace(ss.str())+sumOfOddPlace(ss.str()))%10);  //與整除10相差幾個數

        int EvenDouble[]={0,2,4,6,8,1,3,5,7,9};  //製作偶數表
        int EvenMaxAdd[]={9,7,5,3,1,8,6,4,2,0};
            ...
            if(i%2==0){   //在偶數位的陣列
                if( ((int)strrandnumber[i]-48) < 9 ){
                    if(Complement < EvenMaxAdd[ ((int)strrandnumber[i]-48) ]){  //如果要加的數小於最大可加的值
                        ...
                        /*for(int j=0;j<10;j++){
                            if(EvenDouble[j]==EvenDouble[((int)strrandnumber[i]-48)]+EvenMaxAdd[((int)strrandnumber[i]-48)]){
                                strrandnumber[i]=j+'0';
                                break;
                            }
                        }
                        Complement-=EvenMaxAdd[((int)strrandnumber[i]-48)];*/
...
int generated(const string& buffer){
...
int main(){
    ...
    struct timeval end;
    gettimeofday(&start,NULL);  //程式開始執行時間*/
    ...
    if (!ifs.is_open()) {
        ...
        while (std::getline(ifs, buffer)) {
            ...
            if(length>=13&&length<=16){
                ...
                gettimeofday(&end,NULL);  //程式結束執行時間
